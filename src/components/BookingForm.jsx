    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-semibold">Schedule a Trivia Night</h3>
        ...
        <form onSubmit={submit} className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" name="date" value={form.date} onChange={handleChange} required className="mt-1 w-full rounded border px-3 py-2" />
          </div>
          <div>
            <label className="text-sm text-gray-700">Start time</label>
            <input type="time" name="start_time" value={form.start_time} onChange={handleChange} required className="mt-1 w-full rounded border px-3 py-2" />
          </div>
          <div className="md:col-span-2">
            <label className="text-sm text-gray-700">Location (building)</label>
            <input type="text" name="location" value={form.location} onChange={handleChange} required placeholder="WeWork – 123 Main St" className="mt-1 w-full rounded border px-3 py-2" />
          </div>
          <div>
            <label className="text-sm text-gray-700">Expected attendance (optional)</label>
            <input type="number" name="expected_attendance" value={form.expected_attendance} onChange={handleChange} min="0" className="mt-1 w-full rounded border px-3 py-2" />
          </div>
          <div>
          ...
          <div>
            ...
            <select name="format" value={form.format} onChange={handleChange} className="mt-1 w-full rounded border px-3 py-2">
          ...
          <div className="md:col-span-2 flex items-center justify-end gap-3 mt-2">
            ...
            <button disabled={loading} className="px-4 py-2 rounded bg-blue-600 text-white disabled:opacity-60">{loading ? 'Submitting...' : 'Schedule Event'}</button>
          </div>
        </form>
